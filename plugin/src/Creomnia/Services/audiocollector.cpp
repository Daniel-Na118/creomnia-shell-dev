...
Q_LOGGING_CATEGORY(lcAc, "creomnia.services.ac", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAcWorker, "creomnia.services.ac.worker", QtInfoMsg)

namespace creomnia::services {
...
PipeWireWorker::PipeWireWorker(std::stop_token token, AudioCollector* collector)
    ...
    m_stream = pw_stream_new_simple(pw_main_loop_get_loop(m_loop), "creomnia-shell", props, &events, this);
    if (!m_stream) {
